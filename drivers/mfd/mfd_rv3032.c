...
static int mfd_rv3032_eeprom_wait_busy_max(const struct device *dev, int poll_ms, int64_t max_ms, uint8_t* out_eef)
{
	uint8_t status = 0;
	int err;
...
int mfd_rv3032_enter_eerd(const struct device *dev)
{
	...
	ret = mfd_rv3032_update_reg8(dev, RV3032_REG_TEMPERATURE_LSB, RV3032_TEMPERATURE_EEF,
				 0);
...
static int mfd_rv3032_init(const struct device *dev)
{
	...
	err = mfd_rv3032_eeprom_wait_busy_max(dev, RV3032_EEBUSY_UPDATE_POLL_MS, remaining_time_ms + 2 * RV3032_EEBUSY_WRITE_POLL_MS, NULL);
	if (err) {
	...
	err = mfd_rv3032_update_cfg(dev, RV3032_REG_EEPROM_PMU,
				RV3032_EEPROM_PMU_TCR | RV3032_EEPROM_PMU_TCM | RV3032_EEPROM_PMU_BSM,
				config->backup);
	if (err) {
