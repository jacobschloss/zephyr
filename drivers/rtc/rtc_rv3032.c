...
static int rv3032_configure_clkout(const struct device *dev, uint32_t freq)
{
	...
	err = mfd_rv3032_update_cfg(config->mfd, RV3032_REG_EEPROM_PMU, RV3032_EEPROM_PMU_NCLKE, pmu_reg);
	if (err) {
	...
	err = mfd_rv3032_update_cfg(config->mfd, RV3032_REG_EEPROM_CLKOUT2, 
		RV3032_EEPROM_CLKOUT2_OS | RV3032_EEPROM_CLKOUT2_FD | RV3032_EEPROM_CLKOUT2_HFD_HIGH, 
		clkout2_reg);
	if (err) {
